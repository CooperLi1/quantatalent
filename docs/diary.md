...
- Locked decisions with the user: OpenAI/GPT; Supabase magic-link admin auth;
  lightweight anti-abuse (honeypot + Postgres rate-limiting); placeholder
...
## Server libraries
- DAL (service-role, `server-only`), env accessor, Supabase admin + SSR auth
  clients, crypto (token gen + sha256 hash + constant-time compare), zod
...
- Admin: magic-link login + callback, dashboard (stats, semantic + AI search,
  expandable profiles, enrich/similar/draft-contact/résumé/delete).

## Verification
- `tsc --noEmit` clean; `next build` green (17 routes). Viewed the hero in a
  live preview and iterated on the Q until it matched the reference.

## Deployment
- (see PROJECT.md §12 for remaining: Vercel deploy + env + Supabase Auth URLs.)
...
- Vercel connector can see Cooper's team (`team_Mb1NYVWemYdfwasn6zQATd5w`) but
  ...
  and configured.
- Hardened admin auth with a Next 16 `proxy.ts` session refresh path, admin
  magic-link rate limits, and a response-bound Supabase callback client so PKCE
  session cookies attach to redirects.
...
- Polished admin/login/privacy surfaces: labels instead of placeholder-only
  inputs, visible search errors, calmer loading labels, a real privacy contact,
